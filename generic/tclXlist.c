                    ...
                    Tcl_Obj     *const objv[])
{
     ...
     * exists.
     */
    varObjPtr = Tcl_GetVar2Ex(interp, varName, NULL, TCL_LEAVE_ERR_MSG);
                     ...
                     Tcl_Obj    *const objv[])
{
    ...
    listVarPtr = Tcl_GetVar2Ex(interp, varName, NULL, TCL_LEAVE_ERR_MSG);
...
TclX_ListInit (Tcl_Interp *interp)
{
    ...
    // lassign is available in TCL 8.6 and TCL9
    // Seems to cause a segfault when defined in TCL9, so leave it out until fixed
